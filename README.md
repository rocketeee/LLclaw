...
<p align="center">
  <strong>一键安装 · 多模型支持 · 多智能体协同 · 实时监控 · 可移植配置</strong>
</p>
...
- **多智能体协同**：6 种编排模式、7 大场景模板、智能分工、实时协同监控
...
- **智能体编排**：多 Agent 角色配置、场景模板、工作流可视化
- **协同监控**：任务追踪、Agent 状态、消息流、成本分析
...
LLclaw 内置多智能体协同调用引擎，支持根据使用场景自动编排多个 Agent 协作完成复杂任务。

#### Agent 角色体系

| 角色 | 职责 | 说明 |
|------|------|------|
| 协调者 (Coordinator) | 任务分解与分配 | 接收用户任务，分析意图，拆解子任务并分配给专家 |
| 专家 (Specialist) | 领域执行 | 具备特定领域能力的执行 Agent（代码、写作、分析等） |
| 审核者 (Reviewer) | 质量审核 | 对输出结果进行质量检查、事实核查、改进建议 |
| 综合者 (Synthesizer) | 结果整合 | 汇总多个 Agent 输出，消除矛盾，生成最终结果 |

#### 编排模式

| 模式 | 适用场景 | 说明 |
|------|----------|------|
| 顺序链 (Sequential) | 流水线任务 | A → B → C，任务按顺序流转 |
| 并行扇出 (Parallel) | 可并行子任务 | A → [B, C, D] → E，子任务并行执行后汇总 |
| 路由分发 (Router) | 条件分流 | 根据任务类型智能路由到对应专家 |
| 层级管理 (Hierarchical) | 复杂协作 | 主 Agent 管理和协调多个子 Agent |
| 辩论共识 (Debate) | 决策支持 | 多 Agent 对同一问题辩论，综合最优方案 |
| MapReduce | 大任务拆分 | 拆分 → 并行处理 → 结果汇总归约 |

#### 预设场景模板

| 场景 | 编排模式 | Agent 角色 |
|------|----------|------------|
| 代码开发 | 层级管理 | 架构师 → 前端/后端/DB → 代码审查 → 测试 |
| 内容创作 | 顺序链 | 调研员 → 写作者 → 编辑 → 事实核查 |
| 数据分析 | 顺序链 | 数据采集 → 分析师 → 可视化 → 报告撰写 |
| 智能客服 | 路由分发 | 路由 → FAQ/技术支持/账务 → 质量审核 |
| 翻译校对 | 顺序链 | 翻译 → 校对 → 本地化专家 |
| 文档生成 | MapReduce | 大纲 → 章节并行撰写 → 编辑整合 → 排版 |
| 决策支持 | 辩论共识 | 调研 → 分析/风控/顾问 → 综合建议 |

每个 Agent 可独立配置不同的大模型，例如代码专家使用 DeepSeek、写作专家使用通义千问、审核专家使用智谱 GLM，充分发挥各模型优势。

#### 协同监控

实时监控面板提供以下多智能体监控能力：

- **任务执行队列**：实时追踪每个子任务的状态、进度、耗时
- **Agent 状态网格**：每个 Agent 的忙碑/空闲/异常状态、已完成任务数、平均延迟
- **消息流日志**：Agent 间的任务分配、结果返回、审核反馈全记录
- **Token 消耗趋势**：输入/输出 Token 实时图表
- **成本分布分析**：按 Agent 统计调用成本
- **响应延迟趋势**：各角色 Agent 的平均响应时间
...
| 智能体编排 | Agent 角色管理、6 种编排模式、7 大场景模板、工作流可视化 |
| 协同监控 | 任务追踪、Agent 状态网格、消息流日志、Token/成本分析 |
| 系统监控 | CPU/内存/网络实时图表、日志查看器、性能指标 |
| 安装部署 | 安装/卸载指南、脚本下载、系统要求检查 |
